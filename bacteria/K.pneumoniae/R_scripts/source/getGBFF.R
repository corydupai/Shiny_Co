#' @title Genome Annotation Retrieval (GBFF)
#' @description  Main retrieval function for GBFF files of an 
#' organism of interest. By specifying the scientific name of an organism of 
#' interest the corresponding gbff file storing the annotation  for the organism 
#' of interest can be downloaded and stored locally. GBFF files can be retrieved 
       ...
       if (!is.element(db, c("refseq", "genbank", "ensembl")))
        ...
        if (db == "ensemblgenomes") {
            ...
            if (!is.na(organism_name))
                    message("Starting GBFF retrieval of '", organism_name, "' from ", db, " ...")
            if (is.na(organism_name))
                    message("Starting GBFF retrieval of '", organism, "' from ", db, " ...")
            message("\n")
        } else {
            message("Starting GBFF retrieval of '", organism, "' from ", db, " ...")
            message("\n")
        ...
        if (is.element(db, c("refseq", "genbank"))) {
            ...
            if (nrow(FoundOrganism) == 0) {
                message(
                    paste0(
                        "----------> No reference GBFF file was found for '",
                        organism, "'. Thus, download for this organism has been omitted.",
                        " Have you tried to specify getGBFF(db = '",db,"', organism = '",organism,"' , reference = FALSE) ?",
                        " Alternatively, you can retrieve GBFF files using the NCBI accession ID or NCBI Taxonomy ID.",
                ...
                #     message(
                #       "Unfortunately no GBFF file could be found for organism '",
                #         organism,
                ...
                if (nrow(FoundOrganism) == 1) {
                    if (file.exists(file.path(
                        path,
                        paste0(local.org, "_genomic_", db, ".gbff.gz")
                    ))) {
                        message(
                            "File ",
                            file.path(
                        ...
                        tryCatch({
                            ...
                            message("GBFF download of ", organism, " is completed!")
            ...
            "' again. The md5 hash between the downloaded file and the file ",
               ...
               message("The md5 hash of file '", md5_file_path, "' matches!")
                    ...
                    if (gunzip) {
                            message(
                                    paste0(
                                            "The *.gbff annnotation file of '",
                                            organism,
                                            "' has been downloaded to '",
                                            path,
                                            "' and has been named '",
                                            paste0(local.org, "_genomic_", db, ".gbff"),
                                            "' ."
                                    )
                            )
                    ...
                    if (gunzip) {
                            ...
                            R.utils::gunzip(file.path(path,
                                                      paste0(local.org, "_genomic_", db, ".gbff.gz")), destname = file.path(path,
                                                                                                                                    paste0(local.org, "_genomic_", db, ".gbff")))
                            
                            
                            output_path <- check_annotation_biomartr(file.path(path,
                                                                paste0(local.org, "_genomic_", db, ".gbff")), remove_annotation_outliers = remove_annotation_outliers)
                            
                            return(output_path)
                    } else {
                        output_path <- check_annotation_biomartr(file.path(path,
                                                            paste0(local.org, "_genomic_", db, ".gbff.gz")), remove_annotation_outliers = remove_annotation_outliers)
                            return(output_path)
                    }
                } else {
                    message(
