...
format.files <- as.data.frame(strsplit(get.files, split=" ", fixed = FALSE, perl = FALSE, useBytes = FALSE)) %>% 
  ...
  gather(key = "name", value = "Run",
         -first.col) %>%
  bind_rows(first.entry) %>% 
  select(Run) %>% 
  filter(Run != 'stdout') %>% 
  mutate(`Filename` = file.path(basedir, "Kallisto_results", Run, "abundance.h5", fsep="/"))

tx_im <- merge(tx_im, format.files, by ="Run")

# Step 3: Filter based on coverage 
kallisto.table <- read.table("/stor/work/Wilke/tingle/WGCNA_expanded/coverage.tsv", header = FALSE, sep = " ")

kallisto.df <- kallisto.table %>% 
  select(V1, V5, V7) %>% 
  # transmute(V5 = as.numeric(gsub(",", "", V5))) %>% 
  # filter(V5 > 2.5e+06) %>% 
  # ggplot(aes(x=V5)) +
  # geom_histogram()
  mutate(coverage = as.numeric(gsub(",", "", V7))/as.numeric(gsub(",", "", V5))) %>% 
  transmute(Run = V1, V5 = as.numeric(gsub(",", "", V5)), coverage=coverage) %>% 
  filter(coverage > 0.6) %>% 
  filter(V5 > 2.5e+06)

tx_im <- merge(tx_im, kallisto.df, by="Run")
 
# Step 4 & 5: 
files.in <- tx_im$Filename 
names(files.in) <- tx_im$Run
...
#head(tx_obj$counts)
coldata <- data.frame(
  strain = tx_im$Manual_Assembly,
  sample = tx_im$Sample,
  run = tx_im$Run)

rownames(coldata) = tx_im$Run
colnames(tx_obj$counts) = tx_im$Run

dds <- DESeqDataSetFromTximport(tx_obj,coldata, ~1)
dds <- estimateSizeFactors(dds)

#library(BiocParallel)
register(BPPARAM = MulticoreParam(workers =8))
...
# dds.out <- DESeq(dds, parallel=TRUE, BPPARAM = MulticoreParam(20))


# system.time(DESeq(dds, parallel=TRUE), gcFirst = TRUE)
  #dds, BPPARAM = MulticoreParam(28), parallel=TRUE, p
                 
                 #BPPARAM = MulticoreParam(workers=28), parallel=TRUE, ) # look into specifying number of threads if possible
...
count.out$locus_tags<-rownames(count.out)
fwrite(count.out,paste0(basedir, 
                       "/", 
                       dir_in,
                       "_DESeq.csv"))
```
